...
class Tile(AbstractTile):
  ...
  def get_vert(self, index_keys):
    return self.get_nested_list_value(self.verts, index_keys)

  def set_vert(self, index_keys, value):
    self.set_nested_list_value(self.verts, index_keys, value)

  def get_face(self, index_keys):
    return self.get_nested_list_value(self.faces, index_keys)

  def set_face(self, index_keys, value):
    self.set_nested_list_value(self.faces, index_keys, value)

  def get_neighbor_vert(self, neighbor_keys, index_keys):
    # See comment about neighbors in AbstractTile
    tile = self.get_neighbor_tile(neighbor_keys)
    if not tile:
      return None
    return tile.get_vert(self.index_path(index_keys, neighbor_keys))
  ...
  def add_vert(self, index_keys, ratio_u, ratio_v, **kwargs):
    ...
    self.create_symmetric_verts(index_keys, ratio_u, ratio_v, **kwargs)
    
    # On the boundary, make sure equivalent vertices are set on neighbor tiles
    self.set_equivalent_neighbor_verts(index_keys, vert, **kwargs)

    return vert

  def create_symmetric_verts(self, index_keys, ratio_u, ratio_v, **kwargs):
    ...
    if not 'symmetry' in kwargs:
      ...
      if self.u_symmetric:
        ...
        u_flip_keys = self.u_flip(index_keys)
        self.add_vert(u_flip_keys, 1.0 - ratio_u, ratio_v,
                      **{**kwargs, **extra_args})
        if self.v_symmetric:
          # Add diagonally across
          uv_flip_keys = self.v_flip(u_flip_keys)
          self.add_vert(uv_flip_keys, 1.0 - ratio_u, 1.0 - ratio_v,
                        **{**kwargs, **extra_args})
      if self.v_symmetric:
        # Add reflection about v
        v_flip_keys = self.v_flip(index_keys)
        self.add_vert(v_flip_keys, ratio_u, 1.0 - ratio_v,
                      **{**kwargs, **extra_args})

  def set_equivalent_neighbor_verts(self, index_keys, vert, **kwargs):
    if 'u_boundary' in kwargs:
      self.set_u_equivalent_vert(index_keys, vert, **kwargs)
    if 'v_boundary' in kwargs:
      self.set_v_equivalent_vert(index_keys, vert, **kwargs)
    if 'corner' in kwargs:
      self.set_u_equivalent_vert(index_keys, vert, **kwargs)
      self.set_v_equivalent_vert(index_keys, vert, **kwargs)
      self.set_uv_equivalent_vert(index_keys, vert, **kwargs)
  ...
  def set_equivalent_vert(self, neighbor_keys, index_keys, vert, **kwargs):
    ...
    tile.set_vert(self.index_path(index_keys, neighbor_keys), vert)

  # Handle vert on left/right boundary
  def set_u_equivalent_vert(self, index_keys, vert, **kwargs):
    u_index = self.u_index(index_keys)
    u_flip_keys = self.u_flip(index_keys)
    self.set_equivalent_vert([u_index], u_flip_keys, vert, **kwargs)

  # Handle vert on top/bottom boundary
  def set_v_equivalent_vert(self, index_keys, vert, **kwargs):
    v_index = self.v_index(index_keys)
    v_flip_keys = self.v_flip(index_keys)
    self.set_equivalent_vert([v_index], v_flip_keys, vert, **kwargs)

  # Handle vert on corner, equivalent to vert on diagonal tile
  def set_uv_equivalent_vert(self, index_keys, vert, **kwargs):
    u_index = self.u_index(index_keys)
    v_index = self.v_index(index_keys)
    u_flip_keys = self.u_flip(index_keys)
    uv_flip_keys = self.v_flip(u_flip_keys)
  ...
  def add_face(self, index_keys, vert_index_keys_list, **kwargs):
    if self.get_face(index_keys):
      return None

    verts = self.get_verts_from_list(vert_index_keys_list)
    if not verts: return None

    face = self.mesh_adaptor.create_face(verts)
    self.set_face(index_keys, face)
    ...
    # if the tile has some kind of symmetry defined
    self.create_symmetric_faces(index_keys, vert_index_keys_list, **kwargs)

    # On the boundary, make sure equivalent faces are set on neighbor tiles
    self.set_equivalent_neighbor_faces(index_keys, face, **kwargs)

    return face

  def get_verts_from_list(self, vert_index_keys_list):
    verts = []
    for vert_index_keys in vert_index_keys_list:
      if isinstance(vert_index_keys[0], list):
        vert = self.get_neighbor_vert(vert_index_keys[0], vert_index_keys[1])
      else:
        vert = self.get_vert(vert_index_keys)
      if not vert: return None
      verts.append(vert)

    return verts
    
  def create_symmetric_faces(self, index_keys, vert_index_keys_list, **kwargs):
    # The 'symmetry' keyword is just to ensure we don't recurse forever
    if not 'symmetry' in kwargs:
      extra_args = { 'symmetry': True}
      if self.u_symmetric:
        # Add reflection about u
        u_flip_keys = self.u_flip(index_keys)
        u_flip_vert_index_keys_list = self.u_flip(vert_index_keys_list)
        self.add_face(u_flip_keys, u_flip_vert_index_keys_list,
                      **{**kwargs, **extra_args})
        if self.v_symmetric:
          # Add diagonally across
          uv_flip_keys = self.v_flip(u_flip_keys)
          uv_flip_vert_index_keys_list = self.v_flip(u_flip_vert_index_keys_list)
          self.add_face(uv_flip_keys, uv_flip_vert_index_keys_list,
                        **{**kwargs, **extra_args})
      if self.v_symmetric:
        # Add reflection about v
        v_flip_keys = self.v_flip(index_keys)
        v_flip_vert_index_keys_list = self.v_flip(vert_index_keys_list)
        self.add_face(v_flip_keys, v_flip_vert_index_keys_list,
                      **{**kwargs, **extra_args})

  def set_equivalent_neighbor_faces(self, index_keys, face, **kwargs):
    if 'u_boundary' in kwargs:
      self.set_u_equivalent_face(index_keys, face, **kwargs)
    if 'v_boundary' in kwargs:
      self.set_v_equivalent_face(index_keys, face, **kwargs)
    if 'corner' in kwargs:
      self.set_u_equivalent_face(index_keys, face, **kwargs)
      self.set_v_equivalent_face(index_keys, face, **kwargs)
      self.set_uv_equivalent_face(index_keys, face, **kwargs)
