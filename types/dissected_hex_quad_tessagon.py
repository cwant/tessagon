...
class DissectedHexQuadTile(Tile):
  ...
  def calculate_verts(self):
    ...
    self.add_vert(['left', 'top', 'u_boundary'], 0, 2.0/3.0, u_boundary=True)

    self.add_vert(['left', 'middle'], 0, 0.5, u_boundary=True)

    self.add_vert(['center', 'middle'], 0.5, 0.5)

    self.add_vert(['center', 'top', 'v_boundary'], 0.5, 1.0, v_boundary=True)
  ...
  def calculate_faces(self):
    ...
    self.add_face(['left', 'top', 'u_boundary'],
                  [['left','top', 'corner'],
                   ['left','top', 'interior'],
                   ['left', 'top', 'u_boundary'],
                   [['left'], ['right', 'top', 'interior']]], u_boundary=True)
